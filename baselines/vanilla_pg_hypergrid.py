...
def run_experiment(cfg: OmegaConf) -> None:
    ...
    final_metrics = final_train_state.metrics_module.get(final_train_state.metrics_state)
    
    for key, value in final_metrics.items():
        if "2d_marginal_distribution" in key:
            plt.figure(figsize=(10, 8))
            
            im = plt.imshow(value, cmap='viridis', interpolation='nearest')
            plt.colorbar(im)
            
            plt.title(f"Final 2D Marginal Distribution\n{key}")
            
            plot_path = os.path.join(dir, f"final_{key.replace('/', '_')}.png")
            plt.savefig(plot_path, dpi=150, bbox_inches='tight')
            plt.close()
            log.info(f"Saved visualization to {plot_path}")
