...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    # Mini-batch splits for TLM: split the trajectory batch into N chunks,
    # one gradient step per chunk (mirrors ppo_baseline_num_splits)
    tlm_num_splits = train_state.config.agent.get("tlm_num_splits", 1)
...
# ------------------------------------------------------------------
    # TLM backward loss on a single chunk of trajectories.
    # (Appendix A.2 of TLM paper: online model used here; target EMA is
    #  used only when computing advantages for the PPO update below.)
    # ------------------------------------------------------------------
    def tlm_backward_loss_fn(model_params, chunk_traj_data):
        """Compute TLM loss: -E[sum_t log PB(s_{t-1}|s_t)] on a trajectory chunk."""
        model_to_call = eqx.combine(model_params, policy_static)
    ...
    # ------------------------------------------------------------------
    # One TLM gradient step on a given trajectory chunk.
    # Shared by all three modes (single, multi-batch, multi-grad-step,
    # and mini-batch splits).
    # ------------------------------------------------------------------
    def single_tlm_grad_step(carry, chunk_traj_data):
        """One TLM gradient step on chunk_traj_data."""
        p_params, tlm_opt_state = carry
        tlm_loss, tlm_grads = eqx.filter_value_and_grad(tlm_backward_loss_fn)(
            p_params, chunk_traj_data
        )
    ...
    # ------------------------------------------------------------------
    # Mini-batch splits helper: one epoch over tlm_num_splits chunks of
    # a fixed trajectory batch, mirroring baseline_epoch_body.
    # Returns (updated_params, updated_opt_state, mean_loss).
    # ------------------------------------------------------------------
    def tlm_splits_epoch(p_params, tlm_opt_state, fixed_traj_data):
        """Run one pass over tlm_num_splits mini-batches of fixed_traj_data."""
        B = fixed_traj_data.obs.shape[0]
        split_size = B // tlm_num_splits
        total_loss = 0.0

        for i in range(tlm_num_splits):
            start = i * split_size
            end = (i + 1) * split_size
            chunk = jax.tree.map(lambda x: x[start:end], fixed_traj_data)
            (p_params, tlm_opt_state), loss_i = single_tlm_grad_step(
                (p_params, tlm_opt_state), chunk
            )
            total_loss += loss_i

        return p_params, tlm_opt_state, total_loss / tlm_num_splits

    # ------------------------------------------------------------------
    # TLM update: three modes controlled by config
    #   Mode A — tlm_n_batches > 1  : collect K fresh batches, one splits-epoch each
    #   Mode B — tlm_n_grad_steps > 1: K splits-epochs on the same batch
    #   Default (both == 1)          : single splits-epoch on existing traj_data
    # tlm_num_splits applies in all modes.
    # ------------------------------------------------------------------
    def do_tlm_update(args):
        p_params, tlm_opt_state, rng = args

        # --- Mode A: K fresh batches, one splits-epoch per batch ---
        def collect_and_step(carry, _):
            ...
            new_p, new_opt, loss_i = tlm_splits_epoch(p_params_inner, tlm_opt_state_inner, traj_i)
            return (new_p, new_opt, rng_inner), loss_i

        # --- Mode B: K splits-epochs on the *same* existing batch ---
        def multi_grad_steps(carry, _):
            p_params_inner, tlm_opt_state_inner = carry
            new_p, new_opt, loss_i = tlm_splits_epoch(
    ...
    # Choose which model provides backward logits for the PPO advantage computation:
...
@hydra.main(config_path="configs/", config_name="PPO_tlm_tfbind", version_base=None)
