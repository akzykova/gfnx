...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    def baseline_loss_fn(
        ...
        sq_error = jnp.where(pad_mask, 0.0, sq_error)
        # FIX: sum over time then mean over the batch, matching the
        # reference `V_loss = (Qt - V).pow(2).sum(0).mean()` -- NOT a flat
        # per-step mean. Longer trajectories now contribute proportionally
        # more total gradient signal, as in the reference implementation.
        return sq_error.sum(axis=1).mean()
    ...
    terminal_mask = stop_gradient(comp_old['terminal_step_mask'])
    valid = jnp.logical_not(pad_mask)
    n_valid = jnp.maximum(valid.sum(), 1)

    # FIX: the bootstrap value right after the terminal action of a
    # trajectory must be exactly 0 (there is nothing beyond it), regardless
    # of what the baseline network happens to output on the padded/sink
    # observation that follows it. Without this, `jnp.roll` silently feeds
    # V(pad_obs) into `deltas` at every trajectory's most important step.
    V_next = jnp.roll(V_current, -1, axis=1).at[:, -1].set(0.0)
    V_next = jnp.where(terminal_mask, 0.0, V_next)
    deltas = scores + V_next - V_current
    deltas = jnp.where(pad_mask, 0.0, deltas)

    # FIX: decouple the GAE(lambda) advantage used for the policy update
    # from the critic regression target. The reference implementation
    # always fits V to the unbiased (lambda=1) Monte-Carlo reward-to-go,
    # even though the advantage itself may use lambda<1 for variance
    # reduction. Reusing `gae_lambda` for both (as before) trains the
    # critic on a biased n-step target, which compounds into noisier /
    # biased advantages on subsequent iterations.
    raw_advantages = jax.vmap(compute_gae, in_axes=(0, None))(deltas, gae_lambda)
    mc_returns = jax.vmap(compute_gae, in_axes=(0, None))(deltas, 1.0)
    value_target = stop_gradient(mc_returns + V_current)
...
@hydra.main(config_path="configs/", config_name="TRPO_hypergrid_fixed", version_base=None)
