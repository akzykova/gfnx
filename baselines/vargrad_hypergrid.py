"""Single-file implementation for VarGrad in Hypergrid environment.

Run the script with the following command:
```bash
python baselines/vargrad_hypergrid.py
```
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    log_pb_traj = stop_gradient(log_pb_traj)
    log_reward_traj = stop_gradient(aux_info["log_gfn_reward"])

    rl_reward = log_pb_traj + log_reward_traj + aux_info["entropy"]
    ...
    def vargrad_loss_fn(
        ...
        current_log_pb_traj: chex.Array,
        current_log_reward_traj: chex.Array,
    ):
        ...
        log_pf_traj = fwd_logprobs_traj.sum(axis=1)  # (B,)

        delta = log_pf_traj - current_log_pb_traj - current_log_reward_traj  # (B,)

        delta_mean = jnp.mean(delta)
        batch_size = delta.shape[0]

        vargrad_loss = (
            0.5 * jnp.sum((delta - delta_mean) ** 2) / (batch_size - 1)
        )

        return vargrad_loss, delta

    policy_epochs = train_state.config.agent.policy_epochs

    def policy_epoch_body(epoch_i, carry):
        p_params, p_opt_state = carry

        (_, _), grads = eqx.filter_value_and_grad(vargrad_loss_fn, has_aux=True)(
            p_params, policy_static, traj_data, env, env_params, log_pb_traj, log_reward_traj,
        )

        updates, new_opt_state = train_state.policy_optimizer.update(
            grads, p_opt_state, p_params
        )
        new_p_params = eqx.apply_updates(p_params, updates)

        return (new_p_params, new_opt_state)

    new_p_params, new_opt_state = jax.lax.fori_loop(
        lower=0,
        upper=policy_epochs,
    ...
    (policy_loss, delta), policy_grads = eqx.filter_value_and_grad(
        vargrad_loss_fn, has_aux=True
    )(
        new_p_params, policy_static, traj_data, env, env_params, log_pb_traj, log_reward_traj,
    )

    policy_new_opt_state = new_opt_state
    new_model = eqx.combine(new_p_params, policy_static)
...
@hydra.main(config_path="configs/", config_name="vargrad_hypergrid", version_base=None)
def run_experiment(cfg: OmegaConf) -> None:
    ...
    if cfg.logging.use_writer:
        ...
        writer.init(
            ...
            tags=["TB", env.name.upper(), "VarGrad"],
