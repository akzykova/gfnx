...
TLM modifications:
- tlm_backward_learning_rate: supports {lr, 0.1*lr} sweep via config
- tlm_use_target_ema: EMA target network for backward policy (stabilises training per TLM paper)
- tlm_ema_tau: soft-update coefficient for target EMA
- tlm_n_batches: collect K fresh batches and do one TLM update per batch
- tlm_n_grad_steps: do K gradient steps on TLM with the *same* batch
  (exactly one of tlm_n_batches > 1 OR tlm_n_grad_steps > 1 should be >1)
"""
...
class TrainState(NamedTuple):
    ...
    # TLM EMA target network for backward policy (TLM paper Appendix A.2)
    tlm_target_model: MLPPolicy


def _ema_update(target_params, online_params, tau: float):
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    tlm_use_target_ema = train_state.config.agent.get("tlm_use_target_ema", False)
    tlm_ema_tau = train_state.config.agent.get("tlm_ema_tau", 0.005)
    # K batches: collect K fresh batches, one TLM update per batch
    tlm_n_batches = train_state.config.agent.get("tlm_n_batches", 1)
    # K grad steps: reuse the same batch for K gradient steps
    tlm_n_grad_steps = train_state.config.agent.get("tlm_n_grad_steps", 1)
    ...
    def policy_loss_fn(
        ...
        ppo_clip = jnp.where(pad_mask, 0.0, ppo_clip)

        log_pf_new_full = log_pf_new_full[:, :-1]
        kl = jnp.exp(log_pf_new_full) * (log_pf_new_full - log_pf_old_full)
        kl = jnp.where(pad_mask[..., None], 0.0, kl).sum(axis=-1)
    ...
    # ------------------------------------------------------------------
    # TLM backward loss — uses target EMA model for PB when requested
    # (Appendix A.2 of TLM paper: use target network in the forward loss,
    #  but online network in the TLM loss itself)
    # ------------------------------------------------------------------
    def tlm_backward_loss_fn(model_params, fixed_traj_data):
        """Compute TLM loss: -E[sum_t log PB(s_{t-1}|s_t)]."""
        model_to_call = eqx.combine(model_params, policy_static)
    ...
    # ------------------------------------------------------------------
    # TLM update: two modes controlled by config
    #   Mode A — tlm_n_batches > 1: collect K fresh batches, one grad step each
    #   Mode B — tlm_n_grad_steps > 1: K grad steps on the same (already collected) batch
    # ------------------------------------------------------------------
    def single_tlm_grad_step(carry, fixed_traj_data):
        """One TLM gradient step. Used in both modes via lax.scan."""
        p_params, tlm_opt_state = carry
        tlm_loss, tlm_grads = eqx.filter_value_and_grad(tlm_backward_loss_fn)(
            p_params, fixed_traj_data
        )
    ...
    def do_tlm_update(args):
        p_params, tlm_opt_state, rng = args

        # --- Mode A: K fresh batches, 1 TLM step per batch ---
        def collect_and_step(carry, _):
            p_params_inner, tlm_opt_state_inner, rng_inner = carry
            rng_inner, key_i = jax.random.split(rng_inner)
            traj_i, _ = gfnx.utils.forward_rollout(
                rng_key=key_i,
                num_envs=num_envs,
                policy_fn=fwd_policy_fn,
                policy_params=p_params_inner,
                env=env,
                env_params=env_params,
            )
            (new_p, new_opt), loss_i = single_tlm_grad_step(
                (p_params_inner, tlm_opt_state_inner), traj_i
            )
            return (new_p, new_opt, rng_inner), loss_i

        # --- Mode B: K grad steps on the *same* batch ---
        def multi_grad_steps(carry, _):
            p_params_inner, tlm_opt_state_inner = carry
            (new_p, new_opt), loss_i = single_tlm_grad_step(
                (p_params_inner, tlm_opt_state_inner), traj_data
            )
            return (new_p, new_opt), loss_i

        # Select mode based on config
        # We always run exactly max(tlm_n_batches, tlm_n_grad_steps) steps,
        # but which data is used differs.
        use_multi_batch = tlm_n_batches > 1

        def run_multi_batch(_):
            (final_p, final_opt, _), losses = jax.lax.scan(
                collect_and_step,
                (p_params, tlm_opt_state, rng),
                xs=None,
                length=tlm_n_batches,
            )
            return final_p, final_opt, losses.mean()

        def run_multi_grad(_):
            (final_p, final_opt), losses = jax.lax.scan(
                multi_grad_steps,
                (p_params, tlm_opt_state),
                xs=None,
                length=tlm_n_grad_steps,
            )
            return final_p, final_opt, losses.mean()

        final_p, final_opt, mean_loss = jax.lax.cond(
            use_multi_batch,
            run_multi_batch,
            run_multi_grad,
            None,
        )
        return final_p, final_opt, mean_loss

    def skip_tlm_update(args):
        p_params, tlm_opt_state, _ = args
        return p_params, tlm_opt_state, jnp.array(0.0)

    rng_key, tlm_rng = jax.random.split(rng_key)
    policy_params_after_tlm, tlm_opt_state_after, tlm_loss = jax.lax.cond(
        use_tlm,
        do_tlm_update,
        skip_tlm_update,
        (policy_params, train_state.tlm_backward_opt_state, tlm_rng),
    )

    # ------------------------------------------------------------------
    # EMA target update for backward policy
    # The *forward* policy loss uses the target model's backward logits
    # (frozen), while TLM uses the online model (already updated above).
    # ------------------------------------------------------------------
    target_model_params, _ = eqx.partition(train_state.tlm_target_model, eqx.is_array)

    new_target_params = jax.lax.cond(
        use_tlm & tlm_use_target_ema,
        lambda _: _ema_update(target_model_params, policy_params_after_tlm, tlm_ema_tau),
        lambda _: target_model_params,
        None,
    )
    new_target_model = eqx.combine(new_target_params, policy_static)

    # Choose which params provide backward logits for the PPO advantage computation:
    # - with EMA: use target (frozen) backward policy  (TLM paper Appendix A.2)
    # - without EMA: use the freshly TLM-updated online model
    # NOTE: jax.lax.cond branches must return JAX arrays (pytrees), not equinox
    # modules directly, so we select between parameter pytrees and combine after.
    params_for_adv = jax.lax.cond(
        use_tlm & tlm_use_target_ema,
        lambda _: new_target_params,
        lambda _: policy_params_after_tlm,
        None,
    )
    model_for_adv = eqx.combine(params_for_adv, policy_static)
    ...
    return train_state._replace(
        ...
        tlm_target_model=new_target_model,
        metrics_state=metrics_state,
        eval_info=eval_info,
    )


@hydra.main(config_path="configs/", config_name="PPO_tlm_hypergrid", version_base=None)
def run_experiment(cfg: OmegaConf) -> None:
    ...
    # TLM target model: initialised as a copy of the online model
    # (zero last-layer init is recommended by TLM paper but we replicate
    #  the online init here for simplicity; users can add zero-init separately)
    tlm_target_model = model  # same pytree structure and initial weights
    ...
    # tlm_backward_learning_rate: sweep over {lr, 0.1*lr} via config
    ...
    tlm_backward_optimizer = optax.adam(learning_rate=tlm_backward_schedule)
    ...
    train_state = TrainState(
        ...
        tlm_target_model=tlm_target_model,
    )
    ...
    def train_step_wrapper(idx: int, current_train_state_params) -> TrainState:
    ...
    final_train_state_params = jax.lax.fori_loop(
        lower=0,
        upper=cfg.num_train_steps,
        body_fun=train_step_wrapper,
        init_val=loop_init_val,
    )
