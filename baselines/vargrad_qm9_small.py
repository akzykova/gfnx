"""Single-file implementation for Vargrad in QM9Small environment.

Run the script with the following command:
```bash
python baselines/vargrad_qm9_small.py
```
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    log_reward_traj = stop_gradient(log_info["log_gfn_reward"])
    ...
    new_p_params, new_opt_state = jax.lax.fori_loop(
        ...
        init_val=(policy_params, train_state.opt_state),
    )
    ...
    return train_state._replace(
        rng_key=rng_key,
        model=new_model,
        opt_state=policy_new_opt_state,
        metrics_state=metrics_state,
        eval_info=eval_info,
    )


@hydra.main(config_path="configs/", config_name="vargrad_qm9_small", version_base=None)
